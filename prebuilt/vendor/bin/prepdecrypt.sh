...
{
	log_print 1 "Waiting for KeyMint/Gatekeeper/QSEE services..."
	i=0
	while [ "$i" -lt 25 ]; do
		keymint=$(getprop init.svc.vendor.keymint)
		if [ "$keymint" != "running" ]; then
			keymint=$(getprop init.svc.vendor.keymint-qti)
		fi
		strongbox=$(getprop init.svc.vendor.keymint-strongbox)
		keystore=$(getprop init.svc.keystore2)
		gatekeeper=$(getprop init.svc.vendor.gatekeeper_default)
		qseecomd=$(getprop init.svc.vendor.qseecomd)
		weaver=$(getprop init.svc.vendor.weaver_tms)
		if [ "$keymint" = "running" ] && [ "$keystore" = "running" ] && [ "$gatekeeper" = "running" ] && [ "$qseecomd" = "running" ]; then
			log_print 1 "Crypto services ready: keymint=$keymint strongbox=$strongbox keystore=$keystore gatekeeper=$gatekeeper qseecomd=$qseecomd weaver=$weaver"
			return
		fi
		log_print 2 "Crypto services not ready: keymint=$keymint strongbox=$strongbox keystore=$keystore gatekeeper=$gatekeeper qseecomd=$qseecomd weaver=$weaver"
		sleep 1
		i=$((i + 1))
	done
	log_print 0 "Crypto services wait timed out; continuing anyway."
}
